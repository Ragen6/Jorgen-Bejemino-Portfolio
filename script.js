...
function openResumeModal() {
  try {
    ...
    if (!modal) {
      console.error('âŒ CRITICAL: resumeModal element not found in DOM!');
    ...
    if (modal.classList.contains('active')) {
      console.log('âœ… Active class successfully added to modal');
    } else {
      console.error('âŒ Active class failed to add!');
    }
    
  } catch (error) {
    console.error('âŒ Error in openResumeModal:', error);
  }
}
...
function closeResumeModal() {
  try {
    ...
    if (!modal) {
      console.error('âŒ resumeModal element not found');
      return;
    }
    ...
    console.error('âŒ Error in closeResumeModal:', error);
  }
}
...
window.addEventListener('click', function(event) {
  try {
    ...
    console.error('âŒ Error in window click handler:', error);
  }
});
...
function initializeResumeModalTriggers() {
  try {
    ...
    console.log('âœ… Resume modal triggers initialized:', triggers.length, 'frames');
  } catch (err) {
    console.error('âŒ Failed to initialize resume modal triggers', err);
  }
}
...
document.addEventListener('keydown', function(event) {
  try {
    ...
    console.error('âŒ Error in keydown listener:', error);
  }
});

function initializeFooterMario() {
  const footers = document.querySelectorAll(".footer");
  if (!footers.length) return;

  footers.forEach((footer) => {
    if (footer.querySelector(".mario-track")) return;

    const track = document.createElement("div");
    track.className = "mario-track";
    track.innerHTML = `
      <div class="mario-runner" aria-hidden="true">
        <img src="mario-running.gif" alt="Mario running" class="mario-gif" />
      </div>
      <div class="kirby-runner" aria-hidden="true">
        <img src="kirby-on-a-warp-star.webp" alt="Kirby on a warp star" class="kirby-gif" />
      </div>
      <div class="bowser-runner" aria-hidden="true">
        <img src="bowser-walking.gif" alt="Bowser walking" class="bowser-gif" />
      </div>
    `;
...
document.addEventListener('DOMContentLoaded', function() {
  console.log('ðŸŽ® Page loaded, checking for modal...');
  const modal = document.getElementById('resumeModal');
  if (modal) {
    console.log('âœ… Resume modal element found');
  } else {
    console.error('âŒ WARNING: Resume modal element NOT FOUND');
  }

  initializeFooterMario();
});
